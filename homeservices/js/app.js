(() => {
    ...
    function buildBlogItem(item) {
        let blogItemsTemplate = ``;
        blogItemsTemplate += `<article data-id="${item.id}" class="blog__item item-blog">`;
        item.image ? blogItemsTemplate += `<a href="${item.url}" class="item-blog__image">\n      <img class="ibg" src="${item.image}" alt="Image">\n    </a>` : null;
        blogItemsTemplate += `<div class="item-blog__date">${item.date}</div>`;
        blogItemsTemplate += `<h4 class="item-blog__title">\n      <a href="${item.url}" class="item-blog__link-title">${item.title}</a>\n    </h4>`;
        item.text ? blogItemsTemplate += `<div class="item-blog__text text">${item.text}</div>` : null;
        if (item.tags) {
            blogItemsTemplate += `<div class="item-blog__tags">`;
            for (const tag in item.tags) blogItemsTemplate += `<a href="${item.tags[tag]}" class="item-blog__tag">${tag}</a>`;
            blogItemsTemplate += `</div>`;
        }
        blogItemsTemplate += `</article>`;
        blogItems.insertAdjacentHTML("beforeend", blogItemsTemplate);
    }
