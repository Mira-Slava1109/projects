(() => {
    ...
    (() => {
        ...
        function initSliders() {
            ...
            if (heroSlider) {
                const totalSlides = document.querySelectorAll(".main-slider__slide").length;
                const template = document.getElementById("mini-slide-template");
                let miniSliderInstance = null;
                let mainSliderInstance = null;
                function renderMiniSlides(mainIndex) {
                    const miniWrapper = document.querySelector(".mini-slider .swiper-wrapper");
                    if (!miniWrapper || !template) return;
                    miniWrapper.innerHTML = "";
                    const indexes = [];
                    for (let i = 1; i < totalSlides; i++) indexes.push((mainIndex + i) % totalSlides);
                    indexes.forEach((i => {
                        const slide = document.querySelectorAll(".main-slider__slide")[i];
                        const imgEl = slide?.querySelector(".main-slider__image");
                        if (!imgEl) return;
                        const imgSrc = imgEl.getAttribute("src");
                        const clone = template.content.cloneNode(true);
                        const miniSlide = clone.querySelector(".mini-slide");
                        miniSlide.setAttribute("data-index", i);
                        miniSlide.querySelector("img").setAttribute("src", imgSrc);
                        const topImage = document.createElement("div");
                        topImage.classList.add("mini-slide__top-image");
                        topImage.innerHTML = `<img src="${imgSrc}" alt="Image" />`;
                        const contentEl = miniSlide.querySelector(".mini-slide__content");
                        if (contentEl) contentEl.appendChild(topImage);
                        miniWrapper.appendChild(miniSlide);
                    }));
                    if (miniSliderInstance) miniSliderInstance.update();
                }
                miniSliderInstance = new swiper_core_Swiper(".mini-slider", {
                    slidesPerView: 3,
                    spaceBetween: 36,
                    freeMode: true,
                    grabCursor: true,
                ...
                mainSliderInstance = new swiper_core_Swiper(".main-slider", {
                    ...
                    on: {
                        init(swiper) {
                            swiper.slides.forEach((slide => {
                                const imageEl = slide.querySelector(".main-slider__image");
                                const contentEl = slide.querySelector(".main-slider__content");
                                if (!imageEl || !contentEl) return;
                                const imageSrc = imageEl.getAttribute("src");
                                const topImage = `<div class="main-slider__top-image"><img src="${imageSrc}" alt="Image" /></div>`;
                                contentEl.insertAdjacentHTML("beforeend", topImage);
                            }));
                            renderMiniSlides(swiper.realIndex);
                        },
                        slideChange(swiper) {
                            renderMiniSlides(swiper.realIndex);
                        }
                ...
                document.addEventListener("click", (event => {
                    const clickedSlide = event.target.closest(".mini-slide");
                    if (clickedSlide) {
                        const index = parseInt(clickedSlide.getAttribute("data-index"), 10);
                        if (!isNaN(index) && mainSliderInstance) mainSliderInstance.slideTo(index);
                    }
                }));
            }
